...
![招聘要求](https://cdn.jsdelivr.net/gh/zhang13pro/blogImg/img/20220327170637.png)
...
1. 只会简单地调用 JS 框架的 API，而对于复杂的、非常依赖原生 JavaScript 实现的组件，实现的过程很吃力，甚至难以完成开发需求；
1. 工作中往往需要通过阅读别人的代码去了解当前项目状况，而对于公司内其他人自研的前端组件的实现方式和原理，理解起来又较为困难，更别说在这个基础上进行维护了。
...
并且这其中的一些知识点，比如深浅拷贝和自己手工实现 JSON.Stringify 这样的题目，是大厂面试官经常会问到的内容，所以这部分我将以进阶练习的形式帮你轻松掌握。
...
除此之外，每一个模块的最后我都会专门拿出 1~2 讲来带你实操，手把手帮助你实践，比如实现一个 JSON.Stringify 方法、手写 JS 数组多个 API 的底层实现、实现一个 EventEmitter 等。这种更贴合实践的学习方式，会比你天天抱着编程的书啃，学习效率要高很多。
