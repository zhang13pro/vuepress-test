        ...
        <div data-v-6ecd2302="" class="media-container">
          ...
          <div data-v-6ecd2302="" class="data-wrap inline-block">
            ...
            <div data-v-6ecd2302="" class="detail-wrap">
              <div data-v-6ecd2302="" class="detail-one">40.84M</div>
              <div data-v-6ecd2302="" class="detail-two">00:00/15:39</div>
            </div>
          </div>
    ...
    <div data-v-1605a430="" class="main-container">
      <div data-v-1605a430="" class="rich-text-wrap">
        ...
        <p data-nodeid="1358">
          ...
          TypeScript 增强类型系统，这样上边提到的问题就能迎刃而解了。
        </p>
        <p data-nodeid="1359">
          在 TypeScript
        ...
        <p data-nodeid="1361">
          增强类型系统，顾名思义就是对 TypeScript 类型系统的增强。在 npm
          中，有很多历史悠久的库都是使用 JavaScript 编写的，而 TypeScript 作为
          JavaScript 的超集，设计目标之一就是能在 TypeScript 中安全、方便地使用
          JavaScript 库。
        </p>
        <p data-nodeid="1362">
        ...
        <p data-nodeid="1364">
          ...
          的库呢？关键的步骤就是使用 TypeScript 中的一个 declare 关键字。
        </p>
        <p data-nodeid="1365">
          通过使用 declare
        ...
        <p data-nodeid="1367">
          在运行时，前端代码
          <code data-backticks="1" data-nodeid="1516">&lt;script&gt;</code>
        ...
        <p data-nodeid="1377">
          在上面的例子中，TypeScript
          的报错信息提示：环境声明的上下文不需要实现。也就是说 declare
          声明的所有类型只需要表明类型，不需要实现。
        </p>
        ...
        <p data-nodeid="1382">
          ...
          age，此时我们看到无法访问私有属性 age。另外，我们还声明了方法 getAge
          ，并且 getAge 的返回值是 number 类型，所以 Person
        ...
        <p data-nodeid="1386">
          在上述示例中的第 1~6 行，我们声明了在其他地方定义的枚举 Direction
          类型结构，然后在第 8 行就可以直接访问枚举的成员了（这其实就是我们在 09
        ...
        <p data-nodeid="1392">
          ...
          关键字，我们就可以声明一个内部模块。但是由于 ES6 后来也使用了 module
          关键字，为了兼容 ES6，所以 TypeScript 使用 namespace 替代了原来的
          module，并更名为命名空间。
        </p>
        <blockquote data-nodeid="1393">
          <p data-nodeid="1394">
            <strong data-nodeid="1563">需要注意：目前，任何使用</strong
            ><code data-backticks="1" data-nodeid="1559">module</code
        ...
        <p data-nodeid="1395">
          ...
          的文件都会被当作一个模块。我们可以通过声明模块类型，为缺少 TypeScript
        ...
        <p data-nodeid="1397">
          ...
          方法，然后在 TypeScript 文件中使用了模块 lodash 中的 first 方法。
        </p>
        ...
        <p data-nodeid="1403">
          ...
          关键字导入文件，比如先使用 import 导入图片文件，再通过 webpack
          等工具处理导入的文件。
        </p>
        <p data-nodeid="1404">
        ...
        <p data-nodeid="1415">
          在 TypeScript 中，以 .d.ts 为后缀的文件为声明文件。如果你熟悉
          C/C++，那么可以把它当作 .h 文件。
          在声明文件时，我们只需要定义三方类库所暴露的 API 接口即可。
        </p>
        <p data-nodeid="1416">
        ...
        <p data-nodeid="1462">
          ...
          是由社区人员维护的，如果原来的三方库升级，那么 Definitely Typed
          所导出的三方库的类型定义想要升级还需要经过
          PR、发布的流程，就会导致无法与原库保持完全同步。针对这个问题，在
          TypeScript 中，我们可以通过类型合并、扩充类型定义的技巧临时解决。
        </p>
        ...
        <p data-nodeid="1478">
          在上面的代码中，identity 函数参数为 any 的重载在第一位，因此
          t1、t2、t3 的返回值类型都被重载成了 any。
        </p>
        ...
        <p data-nodeid="1483">
          ...
          age，在第二个命名空间 Person 中没有 age 属性却引用了 age，所以
          TypeScript 报出了找不到 age
          的错误。这是因为非导出成员仅在合并前的命名空间中可见，上例中即 1~6
          行的命名空间中可以访问 age 属性。但是对于 8~12
        ...
        <p data-nodeid="1492">
          在上面的例子中，我们声明了导入模块 person 中 Person 的属性，TypeScript
        ...
        <p data-nodeid="1494">
          在上面的例子中，如果我们删除了扩展模块的声明，第 20 行则会报出
          ts(2339) 不存在 greet 属性的类型错误。
        </p>
        <p data-nodeid="1495">
        ...
        <p data-nodeid="1497">
          全局模块指的是不需要通过 import 导入即可使用的模块，如全局的
          window、document 等。
        </p>
        <p data-nodeid="1498">
        ...
        <p data-nodeid="1500">
          在上面的例子中，因为我们声明了全局的 Array 对象有一个 getLen
          方法，所以为 Array 对象实现 getLen 方法时，TypeScript 不会报错。
        </p>
        ...
        <p data-nodeid="1503">
          在14 讲中，我们将学习 TypeScript 官方提供的工具类型。<strong
            data-nodeid="1694"
