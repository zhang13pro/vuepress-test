  ...
  <div data-v-1605a430="">
        ...
        <div data-v-6ecd2302="" class="media-container">
          ...
          <div data-v-6ecd2302="" class="data-wrap inline-block">
            ...
            <div data-v-6ecd2302="" class="detail-wrap">
              <div data-v-6ecd2302="" class="detail-one">36.85M</div>
              <div data-v-6ecd2302="" class="detail-two">00:00/14:07</div>
            </div>
          </div>
    ...
    <div data-v-1605a430="" class="main-container">
      <div data-v-1605a430="" class="rich-text-wrap">
        ...
        <p data-nodeid="982">
          首先，我们定义了一个 class Dog ，它拥有 string 类型的 name 属性（见第
          2 行）、bark 方法（见第 7 行）和一个构造器函数（见第 3
          行）。然后，我们通过 new 关键字创建了一个 Dog
          的实例，并把实例赋值给变量 dog（见 12
          行）。最后，我们通过实例调用了类中定义的 bark 方法（见 13 行）。
        </p>
        <p data-nodeid="983">
          如果使用传统的 JavaScript
        ...
        <p data-nodeid="985">
          在第 1～ 3 行，我们定义了 Dog 类的构造函数，并在构造函数内部定义了
          name 属性，再在第 4 行通过 Dog 的原型链添加 bark 方法。
        </p>
        <p data-nodeid="986">
          和通过 class
        ...
        <p data-nodeid="991">
          上面的例子展示了类最基本的继承用法。比如第 8 ～12 行定义的<code
            data-backticks="1"
            data-nodeid="1081"
            >Dog</code
          >是派生类，它派生自第 1～6 行定义的<code
            ...
            data-nodeid="1083"
            >Animal</code
          >基类，此时<code data-backticks="1" data-nodeid="1085">Dog</code
          >实例继承了基类<code data-backticks="1" data-nodeid="1087"
            >Animal</code
          >的属性和方法。因此，在第 15～17 行我们可以看到，实例 dog 支持
          bark、say、type 等属性和方法。
        </p>
        ...
        <p data-nodeid="994">
          ...
          基类与第一个例子中的类相比，少了一个构造函数。<strong
            data-nodeid="1094"
            >这是因为派生类如果包含一个构造函数，则必须在构造函数中调用 super()
            方法，这是 TypeScript 强制执行的一条重要规则。</strong
          >
        </p>
        <p data-nodeid="995">
          如下示例，因为第 1～10 行定义的 Dog 类构造函数中没有调用 super
          方法，所以提示了一个 ts(2377) 的错误；而第 12～22 行定义的 Dog
          类构造函数中添加了 super 方法调用，所以可以通过类型检测。
        </p>
        <div class="course-code-area">
          <div class="copy-btn">
            <div class="copy-icon"></div>
        ...
        <p data-nodeid="997">
          有些同学可能会好奇，这里的 super() 是什么作用？其实这里的 super
          函数会调用基类的构造函数，如下代码所示：
        </p>
        <div class="course-code-area">
          <div class="copy-btn">
        ...
        <p data-nodeid="999">
          ...
          的构造函数：constructor Animal(weight: number): Animal 。并且因为
          Animal 类的构造函数要求必须传入一个数字类型的 weight 参数，而第 15
          行实际入参为空，所以提示了一个 ts(2554) 的错误；如果我们显式地给 super
          函数传入一个 number 类型的值，比如说 super(20)，则不会再提示错误了。
        </p>
        ...
        <p data-nodeid="1012">
          ...
          属性是公有的，在任何地方都可见。因此，我们既可以通过第 10 行创建的 Son
          类的实例 son 获取或设置公共的 firstName 的属性（如第 11
          行所示），还可以操作更改 firstName 的值（如第 12 行所示）。
        </p>
        <p data-nodeid="1013">
          不过，对于 private 修饰的私有属性，只可以在类的内部可见。比如第 6
          行，私有属性 lastName 仅在 Son 类中可见，如果其他地方获取了 lastName
          ，TypeScript 就会提示一个 ts(2341) 的错误（如第 14 行）。
        </p>
        <blockquote data-nodeid="1014">
          <p data-nodeid="1015">
            <strong data-nodeid="1113">注意</strong>：TypeScript
            中定义类的私有属性仅仅代表静态类型检测层面的私有。如果我们强制忽略
            TypeScript 类型的检查错误，转译且运行 JavaScript 时依旧可以获取到
            lastName 属性，这是因为 JavaScript 并不支持真正意义上的私有属性。
          </p>
        </blockquote>
        <p data-nodeid="1016">
        ...
        <p data-nodeid="1019">
          ...
          protected，表明此属性在 Son 类及其子类中可见。如示例第 6 行和第 16
          行所示，我们既可以在父类 Son 的构造器中获取 lastName
          属性值，又可以在继承自 Son 的子类 GrandSon 的 getMyLastName 方法获取
          lastName 属性的值。
        </p>
        <blockquote data-nodeid="1020">
          <p data-nodeid="1021">
            ...
            21 行，通过实例的 getMyLastName 方法获取受保护的属性 lastName 是 ok
            的，而第 22 行通过实例直接获取受保护的属性 lastName 则提示了一个
            ts(2445) 的错误。
          </p>
        </blockquote>
        ...
        <p data-nodeid="1025">
          ...
          指定了只读修饰符，这个时候如果再更改 firstName 属性的值，TypeScript
          就会提示一个 ts(2540) 的错误（参见第 9
        ...
        <p data-nodeid="1029">
          除了上边提到的修饰符之外，在 TypeScript 中还可以通过<code
            ...
            data-nodeid="1134"
            >getter</code
          >、<code data-backticks="1" data-nodeid="1136">setter</code
          >截取对类成员的读写访问。
        </p>
        <p data-nodeid="1030">
        ...
        <p data-nodeid="1032">
          在第 14～24 行，我们使用 myLastName 的<code
            ...
            data-nodeid="1140"
            >getter</code
          >、<code data-backticks="1" data-nodeid="1142">setter</code
          >重写了之前的 GrandSon 类的方法，在 getter 中实际返回的是 lastName
          属性。然后，在 setter 中，我们限定仅当 lastName 属性值为 'Tony'
          ，才把入参 name 赋值给它，否则打印错误。<br />
        ...
        <p data-nodeid="1033">
          ...
          'Tony'，所以在第 29 行我们可以把 'Rogers' 赋值给 setter 。而 grandSon1
          实例的 lastName 属性在第 32 行被初始化为 'Tony1'，所以在第 33 行把
          'Rogers' 赋值给 setter 时，打印了我们自定义的错误信息。
        </p>
        ...
        <p data-nodeid="1038">
          ...
          就可以直接访问类上的静态属性和方法了，比如第 8 行访问的是静态属性
          displayName，第 9～10 行访问的是静态方法 isArray。
        </p>
        <p data-nodeid="1039">
          基于静态属性的特性，我们往往会把与类相关的常量、不依赖实例 this
        ...
        <blockquote data-nodeid="1040">
          <p data-nodeid="1041">
            <strong data-nodeid="1179"
              ...
              上下文的方法就可以定义成静态方法，这就意味着需要显式注解 this
              类型才可以在静态方法中使用 this；非静态方法则不需要显式注解 this
              类型，因为 this 的指向默认是类的实例。</strong
            >
          </p>
        </blockquote>
        ...
        <p data-nodeid="1046">
          ...
          Adder，并通过<code data-backticks="1" data-nodeid="1184"
            >abstract</code
          >关键字定义了抽象属性<code data-backticks="1" data-nodeid="1186"
            >x</code
          >、<code data-backticks="1" data-nodeid="1188">y</code>及方法<code
            data-backticks="1"
            data-nodeid="1190"
            >add</code
        ...
        <p data-nodeid="1047">
          同时，我们还在抽象类 Adder 中定义了可以被派生类继承的非抽象属性<code
            data-backticks="1"
            data-nodeid="1193"
            >displayName</code
          >和方法<code data-backticks="1" data-nodeid="1195">addTwice</code>。
        </p>
        <p data-nodeid="1048">
          然后，我们在第 12～23 行定义了继承抽象类的派生类 NumAdder，
          并实现了抽象类里定义的 x、y 抽象属性和 add
          抽象方法。如果派生类中缺少对 x、y、add
          这三者中任意一个抽象成员的实现，那么第 12 行就会提示一个 ts(2515)
          错误，关于这点你可以亲自验证一下。
        </p>
        <p data-nodeid="1049">
          抽象类中的其他非抽象成员则可以直接通过实例获取，比如第 26～28
          行中，通过实例 numAdder，我们获取了 displayName 属性和 addTwice 方法。
        </p>
        <p data-nodeid="1050">
        ...
        <p data-nodeid="1054">
          在第 1～5 行，我们定义了一个包含 x、y、add 属性和方法的接口类型（详见
          07 讲），然后在第 6～12 行实现了拥有接口约定的x、y 属性和 add
        ...
        <p data-nodeid="1058">
          在第 1～6 行，我们在定义类 A ，也说明我们同时定义了一个包含字符串属性
          name 的同名接口类型 A。因此，在第 7 行把一个空对象赋值给类型是 A
          的变量 a1 时，TypeScript 会提示一个 ts(2741) 错误，因为缺少 name
          属性。在第 8 行把对象{ name: 'a2' }赋值给类型同样是 A 的变量 a2
          时，TypeScript 就直接通过了类型检查，因为有 name 属性。
        </p>
