<template>
  <div data-v-1605a430="" class="rich-text-wrap">
    <p data-nodeid="18315" class="">
      我们上一讲介绍了 call、apply、bind
      等方法的实现，而在这一讲中我将会带你一起剖析闭包这个难点。
    </p>
    <p data-nodeid="18316">
      JavaScript
      中的闭包是相当重要的概念，并且与作用域相关知识的指向密切相关，在大厂的前端面试过程中经常会被提及。因此希望通过这一讲的学习，你能够彻底掌握它。
    </p>
    ...
    <p data-nodeid="18327">
      JavaScript 的作用域通俗来讲，就是指变量能够被访问到的范围，在 JavaScript
      中作用域也分为好几种，ES5 之前只有全局作用域和函数作用域两种。ES6
      出现之后，又新增了块级作用域，下面我们就来看下这三种作用域的概念，为闭包的学习打好基础。
    </p>
    <h4 data-nodeid="18328">全局作用域</h4>
    <p data-nodeid="18329">
      在编程语言中，不论 Java 也好，JavaScript
      也罢，变量一般都会分为全局变量和局部变量两种。那么变量定义在函数外部，代码最前面的一般情况下都是全局变量。
    </p>
    <p data-nodeid="18330">
      在 JavaScript 中，全局变量是挂载在 window
    ...
    <div class="course-code-area">
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-keyword">var</span> globalName = <span class="hljs-string">'global'</span>;
    ...
    <p data-nodeid="18332">
      ...
      这个变量无论在什么地方都是可以被访问到的，所以它就是全局变量。而在 getName
      函数中作为局部变量的 name 变量是不具备这种能力的。
    </p>
    <p data-nodeid="18333">
      如果在 JavaScript
      中所有没有经过定义，而直接被赋值的变量默认就是一个全局变量，比如上面代码中
      setName 函数里面的 vName 变量一样。
    </p>
    <p data-nodeid="18334">
      我们可以发现全局变量也是拥有全局的作用域，无论你在何处都可以使用它，在浏览器控制台输入
      window.vName 的时候，就可以访问到 window 上所有全局变量。
    </p>
    <p data-nodeid="18335">
    ...
    <p data-nodeid="18337">
      在 JavaScript
      中，函数中定义的变量叫作函数变量，这个时候只能在函数内部才能访问到它，所以它的作用域也就是函数的内部，称为函数作用域，下面我们来看一段代码。
    </p>
    <div class="course-code-area">
      <div class="copy-btn">
        <div class="copy-icon"></div>
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">getName</span> (<span class="hljs-params"></span>) </span>{
</div></li><li><div class="code-word">  <span class="hljs-keyword">var</span> name = <span class="hljs-string">'inner'</span>;
</div></li><li><div class="code-word">  <span class="hljs-built_in">console</span>.log(name); <span class="hljs-comment">//inner</span>
</div></li><li><div class="code-word">}
</div></li><li><div class="code-word">getName();
    ...
    <p data-nodeid="18339">
      上面代码中，name 这个变量是在 getName 函数中进行定义的，所以 name
      是一个局部的变量，它的作用域就是在 getName
      这个函数里边，也称作函数作用域。
    </p>
    <p data-nodeid="18340">
    ...
    <p data-nodeid="18344">
      听起来好像还不是很能理解块级作用域的意思，那么我们来举个更形象例子，看看到底哪些才是块级作用域呢？其实就是在
      JS 编码过程中 if 语句及 for 语句后面 {...}
      这里面所包括的，就是块级作用域。
    </p>
    ...
    <div class="course-code-area">
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-built_in">console</span>.log(a) <span class="hljs-comment">//a is not defined</span>
    ...
    <blockquote data-nodeid="18351">
      <p data-nodeid="18352">
        红宝书闭包的定义：闭包是指有权访问另外一个函数作用域中的变量的函数。<br />
        MDN：一个函数和对其周围状态的引用捆绑在一起（或者说函数被引用包围），这样的组合就是闭包（closure）。也就是说，闭包让你可以在一个内层函数中访问到其外层函数的作用域。
      </p>
    </blockquote>
    <p data-nodeid="18353">
      乍一看上面的两个比较官方的定义，很难让人理解清晰，尤其是 MDN
      的关于闭包的定义，真的比较让人“头晕”，那么现在就和你说说我是怎么理解的。
    </p>
    <h4 data-nodeid="18354">闭包的基本概念</h4>
    <p data-nodeid="18355">
      我在这里先通俗地讲解一下：闭包其实就是一个可以访问其他函数内部变量的<strong
        data-nodeid="18451"
        >函数</strong
    ...
    <div class="course-code-area">
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">fun1</span>(<span class="hljs-params"></span>) </span>{
    ...
    <p data-nodeid="18358">
      结合闭包的概念，我们把这段代码放到控制台执行一下，就可以发现最后输出的结果是
      1（即 a 变量的值）。那么可以很清楚地发现，a 变量作为一个 fun1
      函数的内部变量，正常情况下作为函数内的局部变量，是无法被外部访问到的。但是通过闭包，我们最后还是可以拿到
      a 变量的值。
    </p>
    <p data-nodeid="18359">
    ...
    <div class="course-code-area">
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-keyword">var</span> a = <span class="hljs-number">1</span>;
    ...
    <p data-nodeid="18364">
      从中可以看出，fun1 函数的作用域指向全局作用域（window）和它自己本身；fun2
    ...
    <p data-nodeid="18366">
      由此可见，闭包产生的本质就是：<strong data-nodeid="18465"
        >当前环境中存在指向父级作用域的引用</strong
      >。那么还是拿上的代码举例。
    </p>
    <div class="course-code-area">
      <div class="copy-btn">
    ...
    <p data-nodeid="18368">
      <img
        src="https://s0.lgstatic.com/i/image2/M01/06/F3/Cip5yGAGp-qABNSNAAUwQ25mVao838.png"
        alt="刘烨的js.png"
        data-nodeid="18468"
      />
    </p>
    ...
    <p data-nodeid="18370">
      那是不是只有返回函数才算是产生了闭包呢？其实也不是，回到闭包的本质，<strong
        data-nodeid="18475"
        >我们只需要让父级作用域的引用存在即可</strong
      >，因此还可以这么改代码，如下所示。
    </p>
    <div class="course-code-area">
      <div class="copy-btn">
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-keyword">var</span> fun3;
</div></li><li><div class="code-word"><span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">fun1</span>(<span class="hljs-params"></span>) </span>{
</div></li><li><div class="code-word">  <span class="hljs-keyword">var</span> a = <span class="hljs-number">2</span>
    ...
    <p data-nodeid="18376">
      <strong data-nodeid="18484">1</strong>.
      返回一个函数，上面讲原因的时候已经说过，这里就不赘述了。
    </p>
    <p data-nodeid="18377">
      <strong data-nodeid="18489">2</strong>. 在定时器、事件监听、Ajax 请求、Web
      Workers
      或者任何异步中，只要使用了回调函数，实际上就是在使用闭包。请看下面这段代码，这些都是平常开发中用到的形式。
    </p>
    <div class="course-code-area">
      <div class="copy-btn">
        <div class="copy-icon"></div>
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-comment">// 定时器</span>
    ...
    <p data-nodeid="18379">
      <strong data-nodeid="18494">3</strong>.
      作为函数参数传递的形式，比如下面的例子。
    </p>
    <div class="course-code-area">
      <div class="copy-btn">
    ...
    <p data-nodeid="18381">
      <strong data-nodeid="18499">4</strong>.
    ...
    <div class="course-code-area">
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-keyword">var</span> a = <span class="hljs-number">2</span>;
    ...
    <div class="course-code-area">
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-keyword">for</span>(<span class="hljs-keyword">var</span> i = <span class="hljs-number">1</span>; i &lt;= <span class="hljs-number">5</span>; i ++){
    ...
    <ol data-nodeid="18390">
      <li data-nodeid="18391">
        <p data-nodeid="18392">
          setTimeout 为宏任务，由于 JS 中单线程 eventLoop
          机制，在主线程同步任务执行完后才去执行宏任务，因此循环结束后
          setTimeout 中的回调才依次执行。
        </p>
      </li>
      <li data-nodeid="18393">
        <p data-nodeid="18394">
          因为 setTimeout 函数也是一种闭包，往上找它的父级作用域链就是
          window，变量 i 为 window 上的全局变量，开始执行 setTimeout 之前变量 i
          已经就是 6 了，因此最后输出的连续就都是 6。
        </p>
      </li>
    </ol>
    <p data-nodeid="18395">
    ...
    <div class="course-code-area">
      ...
      <pre><code data-language="javascript"><ol><li><div class="code-word"><span class="hljs-keyword">for</span>(<span class="hljs-keyword">let</span> i = <span class="hljs-number">1</span>; i &lt;= <span class="hljs-number">5</span>; i++){
</div></li><li><div class="code-word">  setTimeout(<span class="hljs-function"><span class="hljs-keyword">function</span>(<span class="hljs-params"></span>) </span>{
</div></li><li><div class="code-word">    <span class="hljs-built_in">console</span>.log(i);
</div></li><li><div class="code-word">  },<span class="hljs-number">0</span>)
</div></li><li><div class="code-word">}
    ...
    <p data-nodeid="18403">
      从上面的代码可以看出，通过 let 定义变量的方式，重新定义 i
      变量，则可以用最少的改动成本，解决该问题。
    </p>
    ...
    <p data-nodeid="18410">
      另外，在日常的前端开发工作中，开发者往往会忽视对这部分知识的系统性学习，其实闭包的使用在日常的
      JavaScript
      编程中经常出现，使用的场景特别多而且复杂。由于闭包会使一些变量一直保存在内存中不会自动释放，所以如果大量使用的话就会消耗大量内存，从而影响网页性能。因此，你更应该深入理解闭包的原理，从而保证交付的代码性能更好。
    </p>
    <p data-nodeid="18411" class="te-preview-highlight">
      下一讲，我将带领你实现一个
      JSON.stringify。同时希望你多动手练习，也欢迎你在下方留言，说说自己的学习感悟，加油！
    </p>
  </div>
</template>
