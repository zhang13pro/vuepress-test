<template>
  <div data-v-1605a430="" class="rich-text-wrap">
    ...
    <p data-nodeid="2085">
      JavaScript 数组的 API 经常会被 JS 开发者频繁使用，在整个 JavaScript
      的学习过程中尤为重要。
    </p>
    <p data-nodeid="2086">
      ...
      JavaScript，它天生的灵活性，又进一步发挥了数组的特长，丰富了数组的使用场景。<strong
        data-nodeid="2221"
        >可以毫不夸张地说，不深入地了解数组，就不足以写好 JavaScript。</strong
      >
    </p>
    <p data-nodeid="2087">
    ...
    <p data-nodeid="2102">
      截至 ES7 规范，数组共包含 33 个标准的 API 方法和一个非标准的 API
    ...
    <p data-nodeid="2105">
      ...
      6 的空数组，用对象字面量的方式是无法创建的，因此只能写成下述代码这样。
    </p>
    ...
    <ul data-nodeid="2108">
      <li data-nodeid="2109">
        <p data-nodeid="2110">
          <strong data-nodeid="2244">new Array(arg1, arg2,…)</strong
          >，参数长度为 0 或长度大于等于 2
          时，传入的参数将按照顺序依次成为新数组的第 0 至第 N 项（参数长度为 0
          时，返回空数组）；
        </p>
      </li>
      <li data-nodeid="2111">
        <p data-nodeid="2112">
          <strong data-nodeid="2249">new Array(len)</strong>，当 len
          不是数值时，处理同上，返回一个只包含 len 元素一项的数组；当 len
          为数值时，len 最大不能超过 32 位无符号整型，即需要小于 2 的 32
          次方（len 最大为 Math.pow(2,32)），否则将抛出 RangeError。
        </p>
      </li>
    </ul>
    <p data-nodeid="2113">
    ...
    <p data-nodeid="2115">
      鉴于数组的常用性，ES6 专门扩展了数组构造器 Array&nbsp;，新增了 2
      个方法：Array.of、Array.from。其中，Array.of 整体用得比较少；而 Array.from
    ...
    <p data-nodeid="2133">
      结果中可以看出 console.log(value, index, this, arguments.length)
      对应的四个值，并且看到 return 的 value 重复了三遍，最后返回的数组为
      ["aaa","bbb","ccc"]。
    </p>
    <p data-nodeid="2134">
      这表明了通过 Array.from
      这个方法可以自己定义加工函数的处理方式，从而返回想要得到的值；如果不确定返回值，则会返回
      undefined，最终生成的也是一个包含若干个 undefined 元素的空数组。
    </p>
    <p data-nodeid="2135">
      实际上，如果这里不指定 this
    ...
    <p data-nodeid="2137">
      ...
      String、Set、Map&nbsp;等，Array.from 统统可以处理，请看下面的代码。
    </p>
    <div class="course-code-area">
      <div class="copy-btn">
        <div class="copy-icon"></div>
    ...
    <p data-nodeid="2139">
      关于数组构造器 Array&nbsp;新增的两个方法就讲解到这，下面接着介绍如何进行
      Array 的判断。
    </p>
    ...
    <p data-nodeid="2144">
      上面这 5 个判断全部为 True，这里应该没什么疑问。实际上，通过
      Object.prototype.toString 去判断一个值的类型，也是模块一的 01
      讲判断数据类型中我给你推荐的方法。
    </p>
    <p data-nodeid="2145">
    ...
    <p data-nodeid="2147">
      数组的基本概念到这里基本讲得差不多了，下面我们就来看看让人眼花缭乱的 30
      多个数组的基本方法。
    </p>
    <h3 data-nodeid="2148">改变自身的方法</h3>
    <p data-nodeid="2149">
      基于 ES6，会改变自身值的方法一共有 9 个，分别为
      pop、push、reverse、shift、sort、splice、unshift，以及两个 ES6 新增的方法
      copyWithin 和 fill。
    </p>
    <p data-nodeid="2150">
    ...
    <p data-nodeid="2152">
      ...
      sort() 方法的内涵比较多，在第 11 讲我会专门讲解，这里先不做过多的介绍了。
    </p>
    <p data-nodeid="2153">
      下面为了让你对这些 API 方法有更深刻的印象，我结合 Leetcode 中的第 88 题
    ...
    <p data-nodeid="2172">
      上面我把不会改变数组的几个方法大致做了一个回顾，其中 includes
      方法需要注意的是，如果元素中有 0，那么在判断过程中不论是 +0 还是 -0
      都会判断为 True，这里的 includes 忽略了 +0 和 -0。
    </p>
    <p data-nodeid="2173">
      另外还有一个值得强调的是<strong data-nodeid="2344"
        >slice 不改变自身，而 splice 会改变自身</strong
      >，你还是需要注意不要记错了。其中，slice 的语法是：arr.slice([start[,
      end]])，而 splice 的语法是：arr.splice(start,deleteCount[, item1[, item2[,
      …]]])。我们可以看到从第二个参数开始，二者就已经有区别了，splice
      第二个参数是删除的个数，而 slice 的第二个参数是 end 的坐标（可选）。
    </p>
    <p data-nodeid="2174">
      此外，lastIndexOf 和 indexOf 基本功能差不多，不过 lastIndexOf
      是从后面寻找元素的下标；而 toSource
    ...
    <p data-nodeid="2192">
      然后是 initialValue（可选的初始值，作为第一次调用回调函数时传给
      previousValue 的值）。
    </p>
    <p data-nodeid="2193">
      光靠文字描述其实看着会比较懵，我们还是通过一个例子来说明 reduce
      的功能到底有多么强大。
    </p>
    <div class="course-code-area">
      <div class="copy-btn">
    ...
    <p data-nodeid="2197">
      <strong data-nodeid="2380">题目：</strong> var arr = [ {name: 'brick1'},
      {name: 'brick2'}, {name: 'brick3'} ]
    </p>
    <p data-nodeid="2198">
      希望最后返回到 arr 里面每个对象的 name 拼接数据为 'brick1, brick2 &amp;
      brick3' ，如果用 reduce 如何实现呢？
    </p>
    <div class="course-code-area">
      <div class="copy-btn">
        <div class="copy-icon"></div>
