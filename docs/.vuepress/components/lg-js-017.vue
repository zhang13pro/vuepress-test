  ...
  <div data-v-1605a430="">
        ...
        <div data-v-6ecd2302="" class="media-container">
          ...
          <div data-v-6ecd2302="" class="data-wrap inline-block">
            ...
            <div data-v-6ecd2302="" class="detail-wrap">
              <div data-v-6ecd2302="" class="detail-one">26.15M</div>
              <div data-v-6ecd2302="" class="detail-two">00:00/10:01</div>
            </div>
          </div>
    ...
    <div data-v-1605a430="" class="main-container">
      <div data-v-1605a430="" class="rich-text-wrap">
        <p data-nodeid="38688">
          我在上一讲带你实现了一个浏览器端的
          EventEmitter，那么这一讲还是继续进阶，我们来实现一个符合 Promise/A+
          规范的 Promise。
        </p>
        <p data-nodeid="38689">
          其实在“<a
            ...
            data-nodeid="38787"
            >14 | 异步编程（中）：如何深入理解异步编程的核心 Promise</a
          >”中，我已经专门介绍了 Promise 的基本原理和使用情况。由于 Promise
          在异步编程中的重要性不言而喻，因此在很多互联网大厂面试中，现场实现
          Promise 相关方法的题目经常会出现，比如 all、race 或者 any 等。
        </p>
        <p data-nodeid="38690">
        ...
        <blockquote data-nodeid="38702">
          <p data-nodeid="38703">
            ...
            whose behavior conforms to this specification.<br />
            “thenable” is an object or function that defines
            a&nbsp;then&nbsp;method.<br />
            “value” is any legal JavaScript value (including&nbsp;undefined, a
            thenable, or a promise).<br />
        ...
        <blockquote data-nodeid="38718">
          <p data-nodeid="38719">
            A promise must be in one of three states: pending, fulfilled, or
            rejected.<br />
        ...
        <ol data-nodeid="38721">
          ...
          <li data-nodeid="38724">
            <p data-nodeid="38725">
              当状态为 pending 状态时，即可以转换为 fulfilled 或者 rejected
              其中之一。
            </p>
          </li>
        ...
        <p data-nodeid="38731">
          关于 then 方法的英文解读和翻译，我直接总结了出来：一个 Promise
          必须拥有一个 then 方法来访问它的值或者拒绝原因。
        </p>
        ...
        <p data-nodeid="38737">
          ...
          执行结束之后必须被调用，最终返回值为 value，其调用次数不可超过一次。而
          onRejected 除了最后返回的是 reason 外，其他方面和 onFulfilled
          在规范上的表述基本一样。
        </p>
        <p data-nodeid="38738"><strong data-nodeid="38856">多次调用</strong></p>
        <p data-nodeid="38739">
          then 方法其实可以被一个 Promise 调用多次，且必须返回一个 Promise
          对象。then 的写法如下所示，其中 Promise1 执行了 then
          的方法之后，返回的依旧是个 Promise2，然后我们拿着 Promise2 又可以执行
          then 方法，而 Promise2 是一个新的 Promise 对象，又可以继续进行 then
          方法调用。
        </p>
        ...
        <p data-nodeid="38746">
          这一步的思路是：Promise 构造函数接受一个 executor 函数，executor
        ...
        <p data-nodeid="38748">
          从上面的代码中可以看出，我们先定义了一个 Promise 的初始状态
          pending，以及参数执行函数 executor，并且按照规范设计了一个 resolve
          回调函数集合数组 onResolvedCallback 以及 一个 reject
          回调函数集合数组，那么构造函数的初始化就基本完成了。
        </p>
        <p data-nodeid="38749">
          接下来我们看看还需要添加什么东西呢？那就是需要在构造函数中完善 resolve
          和 reject 两个函数，完善之后的代码如下。
        </p>
        <div class="course-code-area">
          <div class="copy-btn">
            <div class="copy-icon"></div>
        ...
        <p data-nodeid="38754">
          ...
          之后，把状态改为相应的值，并把对应的 value 和 reason 存在内部的 data
        ...
        <p data-nodeid="38758">
          ...
          的方法，并且还要保持 then 执行之后，返回的依旧是一个 Promise
          方法，还要支持多次调用（上面标准中提到过）。
        </p>
        <p data-nodeid="38759">
        ...
        <p data-nodeid="38761">
          从上面的代码中可以看到，我们在 then 方法内部先初始化了 Promise2
          的对象，用来存放执行之后返回的 Promise，并且还需要判断 then
        ...
        <p data-nodeid="38764">
          ...
          方法。但是标准里提到了，还要支持不同的 Promise 进行交互，关于不同的
          Promise 交互其实<a
            href="https://promisesaplus.com/#point-46"
            data-nodeid="38879"
            >Promise 标准说明</a
          >中有提到。其中详细指定了如何通过 then 的实参返回的值来决定 Promise2
          的状态。
        </p>
        <p data-nodeid="38765">
          关于为何需要不同的 Promise 实现交互，原因应该是 Promise 并不是 JS
          一开始存在的标准，如果你使用的某一个库中封装了一个 Promise
          的实现，想象一下如果它不能跟你自己使用的 Promise
          实现交互的情况，其实还是会有问题的，因此我们还需要调整一下 then
          方法中执行 Promise 的方法。
        </p>
        <p data-nodeid="38766">
          另外还有一个需要注意的是，在 Promise/A+ 规范中，onResolved 和
          onRejected 这两项函数需要异步调用，关于这一点，标准里面是这么说的：
        </p>
        <blockquote data-nodeid="38767">
          <p data-nodeid="38768">
            In practice, this requirement ensures that onFulfilled and
            onRejected execute asynchronously, after the event loop turn in
            which then is called, and with a fresh stack.
          </p>
        </blockquote>
        <p data-nodeid="38769">
          所以我们需要对代码做一点变动，即在处理 Promise 进行 resolve 或者
          reject 的时候，加上 setTimeout(fn, 0)。
        </p>
        <p data-nodeid="38770">
        ...
        <p data-nodeid="38780">
          下一讲开始，我们就要进入一个新的模块——JS 引擎篇，我要带你了解 JS
          引擎中的垃圾回收机制、事件轮询等，这对于写出高性能的 JavaScript
          代码有着很好的帮助。
        </p>
