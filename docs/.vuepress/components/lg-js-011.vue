        ...
        <div data-v-6ecd2302="" class="media-container">
          ...
          <div data-v-6ecd2302="" class="data-wrap inline-block">
            ...
            <div data-v-6ecd2302="" class="detail-wrap">
              <div data-v-6ecd2302="" class="detail-one">23.9M</div>
              <div data-v-6ecd2302="" class="detail-two">00:00/09:09</div>
            </div>
          </div>
    ...
    <div data-v-1605a430="" class="main-container">
      <div data-v-1605a430="" class="rich-text-wrap">
        <p data-nodeid="737" class="">
          我在上一讲为你介绍了用 JS
          实现数组的各种排序，那么这一讲作为进阶，我要带你了解一下 JS
          提供的数组排序的 sort
          方法。数组排序在日常工作中经常会出现，除了上一讲介绍的排序算法外，通过
          sort 方法也可以实现数组的排序，因此这一讲我会带你剖析 JS 数组 sort
          方法的底层实现。
        </p>
        <p data-nodeid="738">
          我们比较常用的是直接调用 JS 的 sort 方法，这一讲会围绕原生 JS
          方法，并结合 V8 排序的代码一起来分析，以便你碰到类似的 JS
          排序问题时能够轻松应对。
        </p>
        ...
        <p data-nodeid="752">
          从上面的执行结果可以看出，如果不加参数，在第二段代码中，21 会排到 4
        ...
        <p data-nodeid="754">
          ...
          参数&nbsp;，那么数组会按照调用该函数的返回值排序，即&nbsp;a 和 b
          是两个将要被比较的元素：
        </p>
        <ul data-nodeid="755">
          <li data-nodeid="756">
            <p data-nodeid="757">
              如果&nbsp;compareFunction（a, b）小于 0，那么 a 会被排列到 b
              之前；
            </p>
          </li>
          <li data-nodeid="758">
        ...
        <p data-nodeid="766">
          这里你需要回顾上一讲说的插入排序和快速排序，接下来我们就对 sort
          源码进行分析。
        </p>
        <h4 data-nodeid="767">底层 sort 源码分析</h4>
        <p data-nodeid="768">
          先大概来梳理一下源码中排序的思路（下面的源码均来自 V8 源码中关于 sort
          排序的摘要，地址：<a
        ...
        <p data-nodeid="781">
          虽然插入排序理论上是平均时间复杂度为 O(n^2) 的算法，快速排序是一个平均
          O(nlogn)
        ...
        <p data-nodeid="782">
          ...
          足够小的时候，快速排序 nlogn 的优势会越来越小。倘若插入排序的 n
        ...
        <p data-nodeid="784">
          ...
          n , 而每一层的复杂度是 O(n)，因此快排这时候会退化成 O(n^2) 级别。
        </p>
        <p data-nodeid="785">
        ...
        <p data-nodeid="788">
          从上面的源码分析来看，当数据量小于 10 的时候用插入排序；当数据量大于
          10 之后采用三路快排；当数据量为 10~1000
        ...
        <p data-nodeid="791">
          那么关于 sort 排序方法我就介绍到这里了。这一讲，我把 JS 中的 sort
          方法详细讲解了一遍，同时又带你剖析了 sort
          方法中内部的原理实现，你需要好好学习排序相关的算法，才能真正理解 V8
          源码中实现的排序逻辑。
        </p>
        <p data-nodeid="792">
        ...
        <p data-nodeid="828">
          这一讲我只是带你剖析了 sort 的源码实现，下一讲我会带你手写 JS
          数组多个方法的底层实现。
        </p>
