<template>
  <div data-v-1605a430="" class="rich-text-wrap">
    ...
    <p data-nodeid="5494">
      这个方法能够站在全局考察你对 JS
      各种数据类型理解的深度，对各种极端的边界情况处理能力，以及 JS
    ...
    <p data-nodeid="5496">
      因此你要想夯实自身 JavaScript 的编程基础，通过实践来实现一些 JS API
      方法，是非常有必要的，所以这一讲我就来帮你搞懂它。
    </p>
    <p data-nodeid="5497">那么，到底什么是 JSON.stringify 方法？</p>
    <h3 data-nodeid="5498">方法基本介绍</h3>
    <p data-nodeid="5499">
      JSON.stringify 是日常开发中经常用到的 JSON 对象中的一个方法，JSON
      对象包含两个方法：一是用于解析成 JSON 对象的 parse()；二是用于将对象转换为
      JSON 字符串方法的 stringify()。下面我们分别来看下两个方法的基本使用情况。
    </p>
    <h4 data-nodeid="5500">JSON.parse</h4>
    <p data-nodeid="5501">
      JSON.parse 方法用来解析 JSON 字符串，构造由字符串描述的 JavaScript
      值或对象。该方法有两个参数：第一个参数是需要解析处理的 JSON
    ...
    <p data-nodeid="5509">
      JSON.stringify 方法是将一个 JavaScript&nbsp;对象或值转换为 JSON
    ...
    <p data-nodeid="5514">
      ...
      带来的变化：通过替换方法把对象中的属性为字符串的过滤掉，在 stringify
    ...
    <ol data-nodeid="5527">
      ...
      <li data-nodeid="5532">
        <p data-nodeid="5533">
          ...
          undefined，symbol，function 作为数组其中一项的情况做了特殊处理；
        </p>
      </li>
      <li data-nodeid="5534">
        <p data-nodeid="5535">
    ...
    <p data-nodeid="5543">
      上面自己实现的这个 jsonStringify 方法和真正的 JSON.stringify
    ...
    <p data-nodeid="5545">
      通过上面这些测试的例子可以发现，我们自己实现的 jsonStringify 方法基本和
      JSON.stringify 转换之后的结果是一样的，不难看出 jsonStringify
      基本满足了预期结果。
    </p>
    <p data-nodeid="5546">本讲的内容也就先介绍到这里。</p>
    <h3 data-nodeid="5547">总结</h3>
    <p data-nodeid="5548">
      这一讲，我利用原理结合实践的方式，带你实现了一个 JSON.stringify
      的方法。从中你可以看到，要想自己实现一个 JSON.stringify
      方法整体上来说并不容易，它依赖很多数据类型相关的知识点，而且还需要考虑各种边界情况。
    </p>
    <p data-nodeid="5549">
      希望你多加实践，如果在面试中也让你当场实现一个 JSON.stringify
      方法，你才能够轻松应对。
    </p>
    <p data-nodeid="5550">
