        ...
        <div data-v-6ecd2302="" class="media-container">
          ...
          <div data-v-6ecd2302="" class="data-wrap inline-block">
            ...
            <div data-v-6ecd2302="" class="detail-wrap">
              <div data-v-6ecd2302="" class="detail-one">25.83M</div>
              <div data-v-6ecd2302="" class="detail-two">00:00/09:54</div>
            </div>
          </div>
    ...
    <div data-v-1605a430="" class="main-container">
      <div data-v-1605a430="" class="rich-text-wrap">
        ...
        <p data-nodeid="892">
          ...
          Object.prototype.toString.call 返回的结果是 '[object
          arguments]'，可以看出来返回的不是 '[object array]'，说明 arguments
          和数组还是有区别的。
        </p>
        <p data-nodeid="893">
          length
        ...
        <p data-nodeid="901">
          ...
          最后返回的是什么，我们先随便找一个页面中<strong data-nodeid="1019"
            >有 form 表单</strong
          >的页面，在控制台中执行下述代码。
        </p>
        <div class="course-code-area">
        ...
        <p data-nodeid="932">
          ...
          length 的属性。从代码中还可以看出，用 typeof 来判断输出的是
          'object'，它自身是不会有数组的 push 方法的，这里我们就用 call
          的方法来借用 Array 原型链上的 push 方法，可以实现一个类数组的 push
          方法，给 arrayLike 添加新的元素。
        </p>
        <p data-nodeid="933">
          从控制台的结果可以看出，数组的 push
          方法满足了我们想要实现添加元素的诉求。我们再来看下 arguments
          如何转换成数组，请看下面这段代码。
        </p>
        <div class="course-code-area">
          <div class="copy-btn">
        ...
        <p data-nodeid="935">
          这段代码中可以看到，还是借用 Array 原型链上的各种方法，来实现 sum
          函数的参数相加的效果。一开始都是将 arguments
          通过借用数组的方法转换为真正的数组，最后都又通过数组的 reduce
          方法实现了参数转化的真数组 args 的相加，最后返回预期的结果。
        </p>
        <h4 data-nodeid="936">ES6 的方法转数组</h4>
        <p data-nodeid="937">
          对于类数组转换成数组的方式，我们还可以采用 ES6 新增的 Array.from
          方法以及展开运算符的方法。那么还是围绕上面这个 sum
          函数来进行改变，我们看下用 Array.from
        ...
        <p data-nodeid="939">
          从代码中可以看出，Array.from 和 ES6 的展开运算符，都可以把 arguments
          这个类数组转换成数组 args，从而实现调用 reduce
        ...
        <p data-nodeid="943">
          综上你可以看到，类数组这节课的知识点与第 4 讲中 apply、call
