        ...
        <div data-v-6ecd2302="" class="media-container">
          ...
          <div data-v-6ecd2302="" class="data-wrap inline-block">
            ...
            <div data-v-6ecd2302="" class="detail-wrap">
              <div data-v-6ecd2302="" class="detail-one">31.38M</div>
              <div data-v-6ecd2302="" class="detail-two">00:00/12:02</div>
            </div>
          </div>
    ...
    <div data-v-1605a430="" class="main-container">
      <div data-v-1605a430="" class="rich-text-wrap">
        <p data-nodeid="1698" class="">
          上一讲我们探讨了 JavaScript
          引擎的内存管理及垃圾回收机制，那么这一讲依然要讨论和 JS 引擎相关的
          Eventloop，我会带你深入理解 JavaScript 引擎所做的工作。
        </p>
        <p data-nodeid="1699">
          我会先讲解浏览器端的 Eventloop
          的运行机制，待你对浏览器端有了一定的理解之后，再带你设计一些 Node.js
          的 Eventloop
        ...
        <p data-nodeid="1708">
          Eventloop 是 JavaScript 引擎异步编程背后需要特别关注的知识点。JS
        ...
        <p data-nodeid="1709">
          <strong data-nodeid="1789"
            >1.调用堆栈（call stack）负责跟踪所有要执行的代码</strong
        ...
        <p data-nodeid="1716">
          通过上面这张图就能很清晰地看出调用栈、事件队列以及 Eventloop
          和它们之间相互配合的关系。
        </p>
        <p data-nodeid="1717">
          那么看完了 JS 引擎的全局流程图，我们再看看 Eventloop
          的内部都有哪些东西呢？简单来说 Eventloop 通过内部两个队列来实现 Event
          Queue 放进来的异步任务。以 setTimeout
          为代表的任务被称为宏任务，放到宏任务队列（macrotask queue）中；而以
        ...
        <p data-nodeid="1729">
          那么初步看完了浏览器中 Eventloop 的情况，我们再来看下在 Node.js
          服务端的 Eventloop 是怎么运作的。
        </p>
        ...
        <blockquote data-nodeid="1732">
          <p data-nodeid="1733">
            <em data-nodeid="1841"
              ...
              provided input script (or drops into the REPL, which is not
              covered in this document) which may make async API calls, schedule
              timers, or call process.nextTick(), then begins processing the
              event loop.</em
            >
          </p>
        </blockquote>
        <p data-nodeid="1734">
          ...
          异步调用，process.nextTick() 方法会开始处理事件循环。下面就是 Node.js
          官网提供的 Eventloop 事件循环参考流程。
        </p>
        <p data-nodeid="1735">
          <img
        ...
        <ul data-nodeid="1737">
          <li data-nodeid="1738">
            <p data-nodeid="1739">
              <strong data-nodeid="1851">Timers 阶段</strong>：这个阶段执行
          ...
          <li data-nodeid="1740">
            <p data-nodeid="1741">
              ...
              >：这个阶段主要执行系统级别的回调函数，比如 TCP 连接失败的回调。
            </p>
          </li>
          <li data-nodeid="1742">
            <p data-nodeid="1743">
              <strong data-nodeid="1861">idle，prepare 阶段</strong>：只是
              Node.js 内部闲置、准备，可以忽略。
            </p>
          </li>
          <li data-nodeid="1744">
            <p data-nodeid="1745">
        ...
        <ul data-nodeid="1747">
          ...
          <li data-nodeid="1750">
            <p data-nodeid="1751">
              <strong data-nodeid="1883">close callbacks 阶段</strong
        ...
        <blockquote data-nodeid="1753">
          <p data-nodeid="1754">
            ...
            Instead, thenextTickQueuewill be processed after the current
            operation is completed, regardless of the current phase of the event
            loop. Here, an&nbsp;operation&nbsp;is defined as a transition from
            the underlying C/C++ handler, and handling the JavaScript that needs
            to be executed.
          </p>
        </blockquote>
        <p data-nodeid="1755">
        ...
        <p data-nodeid="1761">
          渲染原本就不应该出现在 Eventloop 相关的知识体系里，但是因为 Eventloop
          显然是在讨论 JS
          ...
          >
          的出现却把这两件事情给关联起来，你可以看下 RAF 的英文解释：
        </p>
        ...
        <blockquote data-nodeid="1765">
          <p data-nodeid="1766">
            Rendering opportunities are determined based on hardware constraints
            such as display refresh rates and other factors such as page
            performance or whether the page is in the background. Rendering
            opportunities typically occur at regular intervals.
          </p>
        </blockquote>
        <p data-nodeid="1767">
        ...
        <p data-nodeid="1769">
          当然 requestAnimationFrame 不是 Eventloop 里的宏任务，或者说它并不在
          Eventloop 的生命周期里，只是浏览器又开放的一个在渲染之前发生的新的
          hook。另外需要注意的是微任务的认知概念也需要更新，在执行 animation
          callback 时也有可能产生微任务（比如 promise 的 callback），会放到
          animation queue 处理完后再执行。所以微任务并不是像之前说的那样在每一轮
        ...
        <p data-nodeid="1772">
          当然为了防止浏览器一直处于繁忙状态，导致 requestIdlecallback
        ...
        <p data-nodeid="1775">
          到这里，你基本就能理解 Eventloop 在不同的端上的情况了。虽然说
          Eventloop 本身并不是一个难理解的概念，但是由于 JS
